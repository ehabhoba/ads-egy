...
import { ArrowLeft, Wand2, LoaderCircle, ClipboardCopy, Check, Facebook, Twitter, Instagram, History, RefreshCw } from 'lucide-react';
...
const SocialPostGeneratorPage: React.FC<SocialPostGeneratorPageProps> = ({ service, lang }) => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('user_id', user.id)
        .eq('generation_type', 'social_post')
  ...
  const handleGenerate = async () => {
    ...
    const toneMap = { professional: lang === 'ar' ? 'احترافية' : 'professional', friendly: lang === 'ar' ? 'ودية' : 'friendly', humorous: lang === 'ar' ? 'فكاهية' : 'humorous' };
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        contents: prompt,
        config: { systemInstruction: "You are an expert digital marketer and copywriter specializing in creating engaging and platform-appropriate social media content." }
      });
      
      const newPost = response.text;
      setGeneratedPost(newPost);
      
      const inputs = { topic, platform, tone };
      const { data, error: dbError } = await supabase
        .from('generation_history')
        .insert([{ user_id: user.id, generation_type: 'social_post', inputs, output: newPost }]);
  ...
  const handleReloadHistory = (item: GenerationHistoryItem) => {
    setGeneratedPost(item.output);
    setTopic(item.inputs.topic);
    // Add fallback to default value to prevent errors if platform is missing
    setPlatform((item.inputs.platform as Platform) || 'Facebook');
    setTone(item.inputs.tone as Tone);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-7xl mx-auto px-4 py-16">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        {/* --- CONTROLS --- */}
        <Card className="lg:col-span-1 rounded-2xl border border-slate-700 bg-slate-900 shadow-xl p-6 h-fit">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {(['Facebook', 'X', 'Instagram'] as Platform[]).map(p => (
                            <Button key={p} onClick={() => setPlatform(p)} variant={platform === p ? 'default' : 'outline'} className={`rounded-lg transition-all ${platform === p ? 'bg-indigo-600 text-white' : 'border-slate-700 text-slate-300'}`}>
                                {p === 'Facebook' && <Facebook className="w-4 h-4 sm:mr-2"/>} {p === 'X' && <Twitter className="w-4 h-4 sm:mr-2"/>} {p === 'Instagram' && <Instagram className="w-4 h-4 sm:mr-2"/>}
        ...
        <div className="lg:col-span-2 grid grid-cols-1 xl:grid-cols-3 gap-8">
            ...
            <div className="xl:col-span-1">
                 <Card className="rounded-2xl border border-slate-700 bg-slate-900 shadow-xl h-full">
